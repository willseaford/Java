...
public class MapJob extends Mapper<LongWritable, Text, Text, IntWritable> { 
    ...
    private String preference ="LEAGUEOFLEGENDS";
	...
	if(line.length==7)
	{
	  ...
	  if(viewers!=0 && !line[6].isEmpty() )
	  {
	      ...
	      {
		if(line[2].toUpperCase().contains(sPreferences[0]) ){
			String times[] = line[6].split(" ");
			String parsedTime = times[0].substring(0, 7);
	        	
			outgoing = preference + " (" + sPreferences[0] + ") :" + parsedTime; 
			context.write(new Text(outgoing), new IntWritable(1));
		}

		if(line[2].toUpperCase().contains(sPreferences[1])) {
			String times[] = line[6].split(" ");
			String parsedTime = times[0].substring(0, 7);
	        	
			outgoing = preference + " (" + sPreferences[1] + ") :" + parsedTime; 
			context.write(new Text(outgoing), new IntWritable(1));
		}
